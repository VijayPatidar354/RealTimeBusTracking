    ...
    <div className="flex min-h-screen items-center justify-center bg-ink-50 px-4 dark:bg-ink-950">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-rose-600 shadow-lg shadow-rose-600/30">
            <ShieldCheck className="h-8 w-8 text-white" />
          </div>
          <h1 className="text-2xl font-bold text-ink-950 dark:text-white">Admin Login</h1>
          <p className="mt-1 text-sm text-ink-500">Platform oversight & monitoring</p>
        </div>
        ...
        <div className="rounded-2xl border border-ink-200 bg-white p-6 shadow-panel dark:border-white/10 dark:bg-ink-900">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@example.com" required
                  className="w-full rounded-xl border border-ink-200 bg-ink-50 py-3 pl-10 pr-4 text-sm text-ink-950 outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500/30 dark:border-white/10 dark:bg-ink-800 dark:text-white" />
              </div>
            </div>
            ...
            <button type="submit" disabled={loading}
              className="mt-2 w-full rounded-xl bg-rose-600 py-3 text-sm font-semibold text-white transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-60">
              {loading ? (
