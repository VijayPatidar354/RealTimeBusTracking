...
function DriverMap({ coords, nextStop, allStops, currentStopOrder }) {
  ...
  useEffect(() => {
    ...
    const withCoords = allStops.filter((s) => s.stop_lat && s.stop_lon);
    if (withCoords.length < 2) return;

    // Remove previous layers
    if (passedLineRef.current) {
      map.removeLayer(passedLineRef.current);
      passedLineRef.current = null;
    }
    if (remainLineRef.current) {
      map.removeLayer(remainLineRef.current);
      remainLineRef.current = null;
    }
  ...
  useEffect(() => {
    ...
    if (nextStopRef.current) {
      map.removeLayer(nextStopRef.current);
      nextStopRef.current = null;
    }
...
export default function DriverDashboard() {
  ...
  const loadRouteStops = useCallback(async () => {
    try {
      ...
      /* non-critical — map still works without polyline */
    }
  }, [token]);
  ...
  useEffect(() => {
    ...
    const onNextStop = (data) => {
      setNextStop((prev) => ({
        ...prev,
        stop_id: data.stop_id ?? prev?.stop_id,
        stop_order: data.next_stop_order,
        stop_name: data.next_stop_name,
        stop_lat: data.stop_lat ?? prev?.stop_lat,
        stop_lon: data.stop_lon ?? prev?.stop_lon,
    ...
    const onWaitingUpdated = (data) => {
      setStops((prev) =>
        prev.map((s) =>
          s.stop_id === data.stop_id
            ? { ...s, waiting_count: data.total_waiting }
            : s,
        ),
      );
      setNextStop((prev) =>
  ...
  const triggerMarkArrived = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        if (data.already_progressed) {
          showToast(`ℹ️ "${stop.stop_name}" was already reached`, "info");
        } else {
          showToast(`✅ Reached: ${stop.stop_name}`, "success");
          setCurrentStopOrder((prev) => prev + 1);
        }
        if (data.trip_status === "completed") setTripDone(true);
        // Re-sync with backend to get the correct next stop
        loadStops();
      }
    } catch (err) {
      showToast(err.message || "Could not mark stop", "error");
      // Re-sync on error too — the DB state may have changed
      loadStops();
    } finally {
      markingRef.current = false;
      setMarking(false);
    }
  }, [nextStop, token, showToast, loadStops]);
  ...
  const canArrive = !tripDone && nextStop && !nextStop.stop_lat;
        ...
        >
          ...
          <div
            style={{
              ...
              padding: "4px 10px",
              borderRadius: 20,
              border: `1px solid ${gpsError ? "rgba(239,68,68,0.3)" : "rgba(34,197,94,0.3)"}`,
              background: gpsError
                ? "rgba(239,68,68,0.08)"
                : "rgba(34,197,94,0.08)",
              maxWidth: 200,
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13, color: "#22c55e" }}>
                      {nextStop.waiting_count}{" "}
                      {nextStop.waiting_count === 1
                        ? "passenger"
                        : "passengers"}{" "}
                      waiting
                    </span>
                  ...
                  {/* Proximity bar */}
                  {nextStop.stop_lat ? (
                    // ── GPS-enabled stop: backend auto-detects arrival, no button needed ──
                    <div style={{ marginTop: 10 }}>
                      ...
                      <div
                        style={{
                          ...
                          fontSize: 11,
                          textAlign: "center",
                          color: arriving ? "#22c55e" : "#6b7280",
                          fontWeight: arriving ? 700 : 400,
                        }}
                      >
                        {arriving
                          ? "✅ Detecting arrival automatically…"
                          : `Get within ${GEOFENCE_RADIUS}m of stop`}
                      </div>
                    </div>
                  ) : (
                    // ── No GPS coords on this stop: manual confirmation is the only path ──
                    <>
                      <div
                        style={{
                          marginTop: 10,
                      ...
                      <button
                        ...
                        style={{
                          ...
                          fontSize: 13,
                          fontWeight: 700,
                          cursor: !marking ? "pointer" : "not-allowed",
                          background: marking
                            ? "rgba(255,255,255,0.08)"
                            : "rgba(255,255,255,0.12)",
                          color: marking ? "#4b5563" : "#fff",
                        }}
                      >
                        {marking ? "Processing..." : "Mark Arrived"}
                      </button>
                    </>
                  )}
                </div>
                        ...
                        >
                          <div
                            style={{
                              ...
                              borderRadius: "50%",
                              flexShrink: 0,
                              background: isNext ? "#f1c40f" : "#374151",
                              boxShadow: isNext ? "0 0 6px #f1c40f88" : "none",
                            }}
                          />
                          ...
                          <div
                            style={{
                              ...
                              fontSize: 11,
                              fontWeight: 700,
                              background: hasWaiting
                                ? "rgba(239,68,68,0.15)"
                                : "rgba(255,255,255,0.06)",
                              color: hasWaiting ? "#ef4444" : "#4b5563",
                              border: `1px solid ${hasWaiting ? "rgba(239,68,68,0.3)" : "rgba(255,255,255,0.08)"}`,
                            }}
                          >
                            {stop.waiting_count}
                          </div>
      ...
      {toast ? (
        <div
          style={{
            ...
            color: toast.type === "error" ? "#fff" : "#0d0f14",
